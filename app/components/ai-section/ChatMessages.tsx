  ...
  return (
    ...
    >
          ...
          >
            <div className={`flex items-end ${message.role === 'assistant' ? '' : 'flex-row-reverse'} gap-3`}>
              ...
              <div className={`flex flex-col ${message.role === 'user' ? 'items-end' : 'items-start'}`}>
                {/** Keep markdown readable against per-role bubble backgrounds. */}
                <div className={`${message.role === 'assistant'
                  ? 'bg-white/85 dark:bg-slate-800 text-gray-900 dark:text-gray-100 border border-gray-200 dark:border-slate-700'
                  : 'bg-indigo-600 text-white border border-indigo-500'}
                             rounded-lg px-4 py-3 max-w-[min(42rem,80vw)] wrap-break-word shadow-sm`}>
                  <Markdown
                    content={message.content}
                    className={message.role === 'user'
                      ? '**:text-white [&_a]:text-indigo-100 [&_strong]:text-white'
                      : '[&_a]:text-indigo-600 dark:[&_a]:text-indigo-300'}
                  />
                </div>
      ...
      {!isLoading && messages.length === 0 && (
        <div className="flex h-full min-h-65 items-center justify-center text-center text-sm text-muted-foreground">
          <div>
            <p className="font-medium text-foreground/80">Start the conversation</p>
            <p className="mt-2">Ask about Darshan&apos;s work, projects, or experience.</p>
          </div>
        </div>
      )}
      ...
      {isLoading && messages.length > 0 && (
        <div ref={messagesEndRef} className="flex items-center justify-center py-6">
          <div className="flex items-center space-x-3">
            ...
            <div className="bg-white/85 dark:bg-slate-800 text-gray-900 dark:text-gray-100
                     rounded-lg px-4 py-3 max-w-[80%] border border-gray-200 dark:border-slate-700 shadow-sm">
