...
export function ChatInputBox({ onSendMessage, isLoading, textareaRef, placeholder = 'Ask about Darshan...' }: ChatInputBoxProps) {
    ...
    <form suppressHydrationWarning onSubmit={handleSubmit} className="flex flex-col space-y-3 p-5
      ...
      <div className="flex items-center space-x-3">
        ...
        <textarea
          ...
          disabled={isLoading}
          className="flex-1 min-h-3 resize-none rounded-border px-4 py-3
      ...
      {!inputValue && !isLoading && (
        <div className="mt-4 flex flex-wrap gap-3">
          <button
            type="button"
            onClick={() => setInputValue('Tell me about Darshan Giri Goswami')}
            className="px-4 py-2 rounded-lg text-sm font-medium
                     bg-white/10 dark:bg-slate-900/10 hover:bg-white/20 dark:hover:bg-slate-900/10
                     border border-white/20 dark:border-slate-800/20
                     transition-all duration-200"
          >
            About me
          ...
          <button
            type="button"
            onClick={() => setInputValue('What projects has Darshan built?')}
            className="px-4 py-2 rounded-lg text-sm font-medium
                     bg-white/10 dark:bg-slate-900/10 hover:bg-white/20 dark:hover:bg-slate-900/10
                     border border-white/20 dark:border-slate-800/20
                     transition-all duration-200"
          ...
          <button
            type="button"
            onClick={() => setInputValue('What technologies does Darshan use?')}
            className="px-4 py-2 rounded-lg text-sm font-medium
                     bg-white/10 dark:bg-slate-900/10 hover:bg-white/20 dark:hover:bg-slate-900/10
                     border border-white/20 dark:border-slate-800/20
                     transition-all duration-200"
          ...
          <button
            type="button"
            onClick={() => setInputValue('Can Darshan help with full stack development?')}
            className="px-4 py-2 rounded-lg text-sm font-medium
                     bg-white/10 dark:bg-slate-900/10 hover:bg-white/20 dark:hover:bg-slate-900/10
                     border border-white/20 dark:border-slate-800/20
                     transition-all duration-200"
          ...
          <button
            type="button"
            onClick={() => setInputValue('Contact Darshan')}
            className="px-4 py-2 rounded-lg text-sm font-medium
                     bg-white/10 dark:bg-slate-900/10 hover:bg-white/20 dark:hover:bg-slate-900/10
                     border border-white/20 dark:border-slate-800/20
                     transition-all duration-200"
